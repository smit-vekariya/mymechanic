...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($num1 == 1) {
		...
		if (password_verify($password, $row['c_password'])) {
			...
			$_SESSION['userphone'] = $row['c_phone'];
    ...
    <div class="form-container">
        <div class="field-container">
            <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="customer-form">
                <h2 class="form-field" style="font-weight: bold; color:white"> Customer Login <i
                        class="bi bi-card-checklist"></i></h2>
                <hr>
                <div class="form-field mb-3">
                    <input type="email" maxlength="50" class="form-control" id="email" name="email"
                    oninvalid="this.setCustomValidity('Enter Valid Email it contains \'@\' or\'.\' ')" onchange="this.setCustomValidity('')" pattern=".+@gmail\.com" placeholder="Enter E-Mail" required="required" />
                </div>
